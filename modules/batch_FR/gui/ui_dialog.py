...
class BatchFRHtmlDialog(QDialog):
    ...
    def _load_page(self) -> None:
        """
        Load the external HTML UI into the webview and push initial context.

        We use a single-shot timer to send the JSON payload after the page has
        had a chance to load and define window.batchFrInit.
        """
    ...
    def _handle_preview(self, data: Dict[str, Any]) -> None:
        """Handle a 'preview' command from JS.

        Expects:
            data["path"]: string path to a rule file on disk.

        Reads the file and sends its contents back into the HTML UI via
        `window.batchFrSetPreview(...)`, attempting to render a structured
        rule preview when the file is JSON.
        """
...
def prompt_batch_fr_run_options(
    ...
    """
    Show the Batch FR rule selection dialog (HTML-based, webview-only).

    Returns:
        {"dry_run": bool, "rules_files": List[Path]} on OK with selection
        None on cancel, when Qt is unavailable, or when the webview API
        cannot be imported.
    """
