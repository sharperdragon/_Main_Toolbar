...
# Remove logic is now delegated to remove_engine
from .remove_engine import (
    RemoveContext,
...
def run_batch_find_replace(
    ...
    * Orchestrate: load rules, search notes, apply, and log (pure in-Anki).
    ...
    # 3) Build remove context (generic remove + field-remove patterns)
    remove_ctx: RemoveContext = build_remove_context(
        cfg=cfg,
    ...
    for idx, rule in enumerate(ready, start=1):
        ...
        per["remove_max_loops"] = remove_ctx.remove_max_loops
        ...
        try:
            ...
            for nid in nids:
                ...
                for field in _resolve_fields(note, rule.fields, cfg.fields_all):
                    ...
                    working = apply_remove_pipeline_to_field(
                        text=before,
                        field_name=field,
                        ctx=remove_ctx,
                        per=per,
